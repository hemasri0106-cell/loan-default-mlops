...
def build_model() -> RandomForestClassifier:
    """Builds the baseline Random Forest model (Stronger Baseline)."""
    logger.info("Building Random Forest model (Stronger Baseline)...")
...
def train_model(
    ...
    logger.info("Starting baseline model training (Random Forest)...")
...
def save_metrics(
    ...
    """Saves predictions CSV, feature importances CSV, and JSON report metrics to results_dir."""
    results_dir.mkdir(parents=True, exist_ok=True)
    
    # 1. Save predictions CSV
    predictions_path = results_dir / "random_forest_val_predictions.csv"
    logger.info(f"Saving validation predictions CSV to: {predictions_path}")
    predictions_df.to_csv(predictions_path, index=False)
    
    # 2. Save feature importances CSV
    importances_path = results_dir / "random_forest_feature_importance.csv"
    logger.info(f"Saving feature importances CSV to: {importances_path}")
    importances_df.to_csv(importances_path, index=False)
    
    # 3. Build and save JSON report
...
def main():
    ...
    try:
        ...
        importances_df = extract_feature_importances(model, feature_names)
        ...
        # Save predictions, importances & metrics JSON
        save_metrics(metrics, predictions_df, importances_df, results_dir, training_time)
        ...
        print("\nMetrics saved to:")
        print(f"  - {results_dir / 'random_forest_metrics.json'}")
        print("Feature importances saved to:")
        print(f"  - {results_dir / 'random_forest_feature_importance.csv'}")
        print("="*45)
        
        logger.info("Random Forest training pipeline execution finished.")
